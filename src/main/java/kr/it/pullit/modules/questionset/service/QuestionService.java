...
public class QuestionService implements QuestionPublicApi {
  ...
  public List<LlmGeneratedQuestionResponse> generateQuestions(QuestionGenerationRequest request) {
    ...
    return callLlmClient(request.questionSetId(), llmPrompt, sourceFileDataBytes,
  ...
  public void saveQuestion(Question question) {
    validateQuestionSetExists(question.getQuestionSet().getId(),
        question.getQuestionSet().getOwner().getId());
  ...
  private void validateQuestionSetExists(Long questionSetId, Long ownerId) {
    questionSetRepository.findByIdAndMemberId(questionSetId, ownerId)
  ...
  private List<byte[]> getSourceFileBytes(List<Long> sourceIds, Long ownerId) {
    return sourceIds.stream().map(sourceId -> sourcePublicApi.getContentBytes(sourceId, ownerId))
        .toList();
  }

  private List<LlmGeneratedQuestionResponse> callLlmClient(Long questionSetId, LlmPrompt llmPrompt,
      List<byte[]> sourceFileDataBytes, Integer questionCount) {
  ...
  private LlmGeneratedQuestionRequest createLlmRequest(LlmPrompt llmPrompt,
      List<byte[]> sourceFileDataBytes, Integer questionCount, String modelName) {
    return new LlmGeneratedQuestionRequest(llmPrompt.value(), sourceFileDataBytes, questionCount,
        modelName);
  }
  ...
  private Question buildQuestionFromRequest(QuestionSet questionSet,
      QuestionCreateRequest requestDto) {
    return new Question(questionSet, requestDto.questionText(), requestDto.options(),
        requestDto.answer(), requestDto.explanation());
  }
  ...
  private void updateQuestionDetails(Question question, QuestionUpdateRequestDto requestDto) {
    question.update(requestDto.questionText(), requestDto.options(), requestDto.answer(),
