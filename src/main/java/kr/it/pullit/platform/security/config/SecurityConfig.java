...
public class SecurityConfig {
  ...
  private static final String[] PUBLIC_ENDPOINTS =
      {"/", "/api/health", "/login/oauth2/code/**", "/oauth/authorize/**",
          "/oauth2/authorization/**", "/auth/refresh", "/auth/logout", "/api/notifications/**"};
  ...
  private static final AuthenticationFailureHandler OAUTH2_FAILURE_HANDLER =
      (request, response, ex) -> {
        LoggerFactory.getLogger("OAuth2Failure").error(
            ...
            ex.getClass().getSimpleName(), ex.getMessage(), request.getParameter("state"),
  ...
  private void applyCommon(HttpSecurity http) throws Exception {
    ...
    http.exceptionHandling(handler -> handler.defaultAuthenticationEntryPointFor(
  ...
  public SecurityFilterChain authSecurityFilterChain(HttpSecurity http) throws Exception {
    applyCommon(http);
    http.authorizeHttpRequests(authorize -> authorize.requestMatchers(PUBLIC_ENDPOINTS).permitAll()
